...
export function useImageManager(store) {
  ...
  const getImageUrl = (fileName: string, storage: string, addTimestamp: boolean): string => {
    const data = useRuntimeConfig().public.wwwUrl + "/" + storage + "/" + fileName + ".gif";
  ...
  const getAvatar = async (): Promise<string> => {
    const data = useRuntimeConfig().public.wwwUrl + "/" + store.$id + "/" + store.currentItem.id + ".gif";
    const defaultavatar = useRuntimeConfig().public.wwwUrl + "/" + store.$id + "/defaultavatar.png";
...
export const getImageFromStorage = (storage: string, fileName: string) => {
  //const supabase = useSupabaseClient();
  try {
    return useRuntimeConfig().public.wwwUrl + "/" + storage + "/" + fileName + ".gif";
  } catch (error) {
    return null;
  }
};
